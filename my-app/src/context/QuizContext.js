...
function quizReducer(state, action) {
  switch (action.type) {
    ...
    case "ADD_IDENTITY_RESPONSE":
      return {
        ...state,
        userIdentityResponses: state.userIdentityResponses.concat(
          action.payload
        ), // Add new response to existing list
      };
...
export function QuizProvider({ children }) {
  ...
  return (
    <QuizContext.Provider value={{ state, dispatch }}>
      {children}{" "}
      {/* All the components inside this provider can access state & dispatch */}
